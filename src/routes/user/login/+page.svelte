	...
	<div class="rounded-xl bg-gray-100 p-5 m-2 mx-auto">
		...
		<form method="post" action="?/register" class="flex flex-col gap-6 m-2" use:enhance>
			<div class="flex flex-row items-center justify-evenly gap-6">
				...
				<input name="username" required type="text" placeholder="Username" bind:value={inputForm.username} class="w-60 rounded-lg p-2 border-2 {validation.errors.some((obj) => obj.type === 'username' || obj.type === 'email') ? 'border-red-500' : 'border-gray-300'} focus:border-orange-500 focus:outline-none" />
			</div>
			<div class="flex flex-row items-center justify-evenly gap-6">
